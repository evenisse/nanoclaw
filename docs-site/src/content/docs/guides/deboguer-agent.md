...
### Corruption persistante d'inbound.db

NanoClaw détecte les erreurs de corruption répétées sur `inbound.db` et arrête proprement le container plutôt que de boucler. Le symptôme : le container se lance, produit une erreur SQLite, et s'arrête sans boucle infinie.

```bash
# Confirmer la corruption
tail -50 logs/nanoclaw.error.log | grep -i "malformed\|corruption\|database disk"
```

La solution est de supprimer la session corrompue. La prochaine interaction recréera une session propre :

```bash
rm -rf data/v2-sessions/<ag-id>/<session-id>/
```

L'agent group et son workspace (`groups/<folder>/`) ne sont pas affectés.
