...
Les réponses d'un agent doivent être enveloppées dans des balises `<message to="...">`, même pour un agent avec une seule destination. Le texte brut hors de ces balises n'est pas livré — il reste dans le scratch pad interne de l'agent.

```xml
<message to="nom-de-la-destination">
Voici ma réponse.
</message>
```

Pour un agent branché sur un seul canal, le nom de destination correspond au nom local configuré (souvent le nom du canal ou le nom de l'agent). L'agent peut toujours l'obtenir avec `ncl destinations list`.
