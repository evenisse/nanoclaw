...
### Export de traces de session

Via la commande `/upload-trace`, l'agent peut exporter le transcript de sa session courante vers Hugging Face. Utile pour le débogage, l'archivage ou la réutilisation de contexte dans d'autres systèmes. La commande est disponible depuis une conversation avec l'agent.

```
Vous : /upload-trace
→ L'agent archive le transcript courant et renvoie l'URL Hugging Face
```
...
Pour créer des skills personnalisées ou contrôler quelles skills sont actives par groupe, voir [Ajouter des skills à un agent](/guides/ajouter-skills/).
