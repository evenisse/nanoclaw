 ...
 * Also forwards real-time events from the event bus to /api/events/push.
 */
import fs from 'fs';
...
import type { DashboardEvent } from './dashboard/event-bus.js';
...
function startEventForward(config: PusherConfig): void {
  dashboardBus.on('event', (event: DashboardEvent) => {
    postJson(config, '/api/events/push', event);
  });
}
...
function collectAgentGroups() {
  return getAllAgentGroups().map((g) => {
    ...
    const containerConfig = db.prepare('SELECT * FROM container_configs WHERE agent_group_id = ?').get(g.id) as
...
function collectContextWindows() {
  ...
  for (const agDir of fs.readdirSync(sessionsDir).filter((d) => d.startsWith('ag-'))) {
    ...
    const walk = (dir: string): void => {
      try {
        for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
          const full = path.join(dir, entry.name);
          if (entry.isDirectory()) walk(full);
          // Include rotated transcripts (.jsonl.rotated-<ts>) so we can fall
          // back to them when the active file was just rotated and is empty.
          else if (entry.name.endsWith('.jsonl') || entry.name.includes('.jsonl.rotated-')) jsonlFiles.push(full);
        }
      } catch {
        /* skip */
      }
    ...
    // Try files newest-first; stop at the first one that contains usage data.
    // This handles the post-rotation window where the active .jsonl is empty.
      ...
      for (let i = lines.length - 1; i >= 0; i--) {
        ...
        try {
          ...
          if (r.type === 'assistant' && r.message?.usage) {
            ...
            const sessionId = basename.replace(/\.jsonl(\.rotated-\d+)?$/, '');
            results.push({
              agentGroupId: agDir,
              agentGroupName: nameMap.get(agDir),
              sessionId,
              rotated: basename.includes('.rotated-'),
              model,
              contextTokens: ctx,
              outputTokens: u.output_tokens || 0,
              cacheReadTokens: u.cache_read_input_tokens || 0,
